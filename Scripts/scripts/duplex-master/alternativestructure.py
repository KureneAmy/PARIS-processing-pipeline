...
for chrom in chroms:
    ...
    for i in intervals[chrom]:
        ...
        if dgcluster and len(dgcluster) > 1: 
            dgclusters.add(dgcluster)
...
def bpcheck(chrom, intvl1, intvl2, fastadict):
    ...
    for intvl in [intvl1, intvl2]: 
        ...
        helix = p.communicate(input = (seq + "\n" + constraint).encode())[0].decode().split('\n')
        ...
        if structure[0][0] != ".": 
            structureintvl[0] = intvl[0]
        else: 
            structureintvl[0] = intvl[0] + len(structure[0].split("(")[0])
        if structure[0][-1] != ".": 
            structureintvl[1] = intvl[1]
        else: 
            structureintvl[1] = intvl[1] - len(structure[0].split("(")[-1])
        if structure[1][0] != ".": 
            structureintvl[2] = intvl[2]
        else: 
            structureintvl[2] = intvl[2] + len(structure[1].split("(")[0])
        if structure[1][-1] != ".": 
            structureintvl[3] = intvl[3]
        else: 
            structureintvl[3] = intvl[3] - len(structure[1].split("(")[-1])
    ...
    if maxoverlap > over_threshold: 
        return maxoverlap, structures[0], structures[1]
    else: 
        return 0, 0, 0
...
for chrom in chroms:
    # Skip if chrom not in fasta dict
    if chrom not in fastadict:
        print(f"Skipping chrom {chrom} - not found in fasta file")
        continue
