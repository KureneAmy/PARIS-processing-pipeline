...
use lib "module";
#use PARISutil qw( &loadGTF_interval &loadGenome &parseCigar &reverseComplement &localAlignment );
...
my %environment = (
    ...
    binPath           => "/mnt1/4.NAS2025/zhangam/easyomics_test/PARIS/scripts/paris-master/bin",
...
my %parameters = (
    samFiles          => "NULL",
    chiasticFiles     => "NULL",
    chiasticSamFiles  => "NULL",
    output            => "",
    genomeFile        => $environment{genomeFile},
    genomeSizeFile    => "",
    annotationFile    => $environment{annotationFile},
    transcriptomeFile => $environment{transcriptomeFile},
    minOverhang       => $environment{minOverhang},
    minSupport        => 1,
    scoringMethod     => "geometric",
    help              => 0,
    verbose           => 0,
    debug             => 0
);
...
## --------------------------------------
 ...
 # more options:
  ...
  --binPath  path to bin directory (contains encode.jar, decode.jar, conngraph, etc.)
...
GetOptions(
    'i=s'        => \$parameters{samFiles},
    'j=s'        => \$parameters{chiasticFiles},
    's=s'        => \$parameters{chiasticSamFiles},
    'o=s'        => \$parameters{output},
    'g=s'        => \$parameters{genomeFile},
    'z=s'        => \$parameters{genomeSizeFile},
    'a=s'        => \$parameters{annotationFile},
    't=s'        => \$parameters{transcriptomeFile},
    'l=i'        => \$parameters{minOverhang},
    'p=i'        => \$parameters{minSupport},
    'c=s'        => \$parameters{scoringMethod},
    'binPath=s'  => \$environment{binPath},
    'h|help'     => \$parameters{help},
    'V|verbose'  => \$parameters{verbose},
    'D|debug'    => \$parameters{debug}
) or die "Error in command line arguments\n$usage";

if ( $parameters{help} || !$parameters{output} || (!$parameters{chiasticFiles} && !$parameters{chiasticSamFiles}) ) {
    print $usage;
    exit 0;
}

$_debug = $parameters{debug};
$environment{minOverhang} = $parameters{minOverhang};
...
sub main
{
    ...
    my $outputFile = $parameters{output} . "." . $parameters{scoringMethod};
    my $supportSamFile = $outputFile;  $supportSamFile =~ s/txt$//;  $supportSamFile .= "sam";

    $global{annotation} = PARISutil::loadGTF_interval ( $parameters{annotationFile} );
...
sub cliqueCollapse
{
    ...
    print STDERR `java -jar $environment{binPath}/encode.jar $cliqueBedConnectFile`;
    print STDERR `$environment{binPath}/conngraph $cliqueBedConnectFile.nv $cliqueBedConnectFile.en > $cliqueBedConnectFile.en.cg`;
    print STDERR `java -jar $environment{binPath}/decode.jar $cliqueBedConnectFile.map $cliqueBedConnectFile.en.cg`;
...
sub readCollapse
{
    ...
    print STDERR `java -jar $environment{binPath}/encode.jar $duplexCollapseFile`;
    print STDERR `$environment{binPath}/conngraph $duplexCollapseFile.nv $duplexCollapseFile.en > $duplexCollapseFile.en.cg`;
    print STDERR `java -jar $environment{binPath}/decode.jar $duplexCollapseFile.map $duplexCollapseFile.en.cg`;
...
sub clique2DuplexGroup
{
    ...
    if ( $parameters{mode} eq "multiple" ) {
        ...
        while ( my $line = <CL> ) {
            ...
            foreach my $read ( @data ) {
                next if ( not $read );
                if ( not defined $ref_read->{$read}{clique} ) { $ref_read->{$read}{clique} = "$cliqueID"; }
